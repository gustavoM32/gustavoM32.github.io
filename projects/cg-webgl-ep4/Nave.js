...
class Nave {
     ...
     * @param {vec4} cor - cor da nave
     */
    constructor(posicao, theta = vec3(0, 0, 0), velocidade = 0, cor = vec4(1.0, 1.0, 1.0, 1.0)) {
        this.pos = posicao;
        this.matrizRotacao = rotateXYZ(theta);
        this.vTrans = velocidade;
        let propsObjeto = {
            posicao: vec3(posicao),
            rotacao: this.pegaAngulosRotacao(),
        ...
        if (Config.consideraEnunciado) {
            this.objeto = new Esfera(propsObjeto);
        } else {
            this.objeto = new NaveObjeto(propsObjeto);
        }
    }
    ...
    imprimeInformacoes() {
        ...
        let [ax, ay, az] = this.pegaAngulosRotacao();

        console.log(`theta: (${Math.round(ax)}°, ${Math.round(ay)}°, ${Math.round(az)}°)`);
    }

    /**
     * Devolve ângulos de rotação.
     */
    pegaAngulosRotacao() {
    ...
    atualizaVisao() {
        ...
        gl.uniformMatrix4fv(gShader.uView, false, safeFlatten(gCtx.vista));

        this.objeto.theta = this.pegaAngulosRotacao();
    }

    /**
     * Desenha objeto que representa a nave.
     */
    desenha() {
        this.objeto.desenha();
    }
